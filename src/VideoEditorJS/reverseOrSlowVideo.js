...
{
    process = "reversing";
    setting.innerHTML = "<video src='' crossorigin id='videoFile' style='display: none;width: 100%;height: 200px;'></video><label for='audio'><h3>Mute Audio</h3></label>&nbsp;&nbsp;<input type='checkbox' id='audio' name='audio' style='width:1em;height:1em;'></input>"
...
console.log = function () {
  ...
  if (CurrentTime != null) {
    ...
    else
    {
        ...
        LandingText.innerHTML = `Please wait ,we are ${process} video in the most secure way<br><span>${
...
var processVideo = async () => {
  ...
  else
  {
    ...
    {
      let atem=1/speed;
      FFMPEGCommand = `-i input.`+type+` -filter_complex [0:v]minterpolate=mi_mode=mci:mc_mode=aobmc:vsbmc=1:fps=120,setpts=`+speed+`*PTS[v];[0:a]atempo=`+atem+`[a] -map [v] -map [a]  Output.`+type;
    }
    else
      FFMPEGCommand = `-i input.`+type+` -filter:v minterpolate=mi_mode=mci:mc_mode=aobmc:vsbmc=1:fps=120,setpts=`+speed+`*PTS -an Output.`+type;
  }
